...
const BaseInformation = (props) => {
  ...
  const [todayGallery, setTodayGallery] = useState(
    finalData?.gallery_start_date || moment()
  );
  const dispatch = useDispatch();
  useEffect(() => {
    let listDate = {};
    finalData?.start_time &&
      (listDate["start_time"] = moment(finalData?.start_time));
    finalData?.gallery_start_date &&
      (listDate["gallery_start_date"] = moment(
        finalData?.gallery_start_date || ""
      ));
    finalData?.end_time
      ? (listDate["end_time"] = moment(finalData?.end_time))
      : (listDate["end_time"] = moment(finalData?.start_time));

    finalData?.gallery_end_date &&
      (listDate["gallery_end_date"] = moment(finalData?.gallery_end_date));
    finalData?.start_clock &&
      (listDate["start_clock"] = moment(finalData?.start_clock));
    finalData?.end_clock &&
      (listDate["end_clock"] = moment(finalData?.end_clock));
    finalData?.gallery_start_clock &&
      (listDate["gallery_start_clock"] = moment(
        finalData?.gallery_start_clock
      ));
    finalData?.gallery_end_clock &&
      (listDate["gallery_end_clock"] = moment(finalData?.gallery_end_clock));
  ...
  const onFinish = (values) => {
    if (media) {
      dispatch(
        setAUCTION({ ...values, media, has_gallery: has_gallery_state })
      );
  ...
  const handleResultUpload = (value) => {
    if (value?.media_path) setMedia(value);
    // dispatch(setAUCTION({media:value}))
  };
  return (
    <>
      <Form onFinish={onFinish} form={form} wrapperCol={{ span: 24 }}>
        <div className="row">
              ...
              <div className="col-md-6">
                <div className="input-group">
                  ...
                  >
                    <textarea
                      className=""
                      placeholder="جزییات حراج را وارد نمایید."
                    />
                  </Form.Item>
                </div>
              </div>
                    ...
                    <div className="input-group">
                      ...
                      >
                        <TimePicker
                          ...
                          format={"HH:mm"}
                          showNow={false}
                          className="default-input custom-timePicker mt-2"
                        />
                      </Form.Item>
                    </div>
                  </div>
                            ...
                            },
                            {
                              validator: (_, value) =>
                                value >= today
                                  ...
                                  : Promise.reject(
                                      new Error(
                                        ` تاریخ بزرگتر از تاریخ شروع وارد کنید `
                                      )
                                    ),
                            },
                          ]}
                        ...
                        <div className="input-group">
                          <div className="form-check">
                            <input
                              ...
                              onChange={
                                (e) => setHas_gallery_state(e.target.checked)
