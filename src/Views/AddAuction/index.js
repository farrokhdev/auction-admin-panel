...
import {
  removeAUCTION,
  setAUCTION,
} from "../../redux/reducers/auction/auction.actions";
...
function Index() {
  ...
  useEffect(() => {
    if (auctionId !== "new") getData();
    if (!id) dispatch(getProfile());
  }, []);

  const getData = (e = "") => {
    setLoading(true);
    axios
      .get(UrlQuery(`${BASE_URL}${DETAIL_AUCTION(auctionId)}`))
      .then((resp) => {
        setLoading(false);
        ...
        if (resp.data.code === 200 && resp.data?.data?.result) {
          ...
          let gallery_start_date = res?.gallery_start_date
            ? res?.gallery_start_date
            : "";
          let gallery_end_date = res?.gallery_end_date
            ? res?.gallery_end_date
            : "";
          let start_clock = res?.start_time.slice(11);
          let end_clock = res?.end_time.slice(11);
          let gallery_start_clock = res?.gallery_start_date
            ? moment(res?.gallery_start_date.slice(11), "HH:mm")
            : moment("08:00", "HH:mm");
          let gallery_end_clock = res?.gallery_end_date
            ? moment(res?.gallery_end_date.slice(11), "HH:mm")
            : moment("20:00", "HH:mm");

          res.auction_product.map(
            (t) =>
          ...
          steps = res.steps.map((t, i, array) => ({
            minimum: i > 0 ? array[i - 1]?.threshold : 0,
            ...t,
          }));
          if (res?.dates_auction?.length) {
            ...
            productsArrayDate = res.auction_product.map((t) => ({
              ...t?.product,
              ...t,
              id: t?.product_id,
            }));
  ...
  const sendData = async (values) => {
    ...
    if (!allDataMain?.gallery_start_date) {
      ...
      allDataMain["gallery_start_date"] =
        (await moment(allDataMain.gallery_start_date).format("YYYY-MM-DD ")) +
        moment(allDataMain.gallery_start_clock).format("HH:mm");
      allDataMain["gallery_end_date"] =
        (await moment(allDataMain.gallery_start_date).format("YYYY-MM-DD ")) +
        moment(allDataMain.gallery_end_clock).format("HH:mm");
    }
    ...
    setLoading(true);
    allData["start_time"] =
      (await moment(allDataMain.start_time).format("YYYY-MM-DD ")) +
      moment(allDataMain.start_clock).format("HH:mm");
    allData["end_time"] = (await (allDataMain?.type !== "LIVE"))
      ? moment(allDataMain.end_time).format("YYYY-MM-DD ") +
        moment(allDataMain.end_clock).format("HH:mm")
      : (await moment(allDataMain.start_time).format("YYYY-MM-DD ")) +
        moment(allDataMain.start_clock).format("HH:mm");
    allDataMain["bidding_interval"] = (await (allDataMain?.type === "ONLINE"))
      ? allDataMain.bidding_interval
      : null;
    ...
    if (auctionId !== "new") {
      await axios
        .put(`${BASE_URL}${EDIT_AUCTION(auctionId)}`, {
          ...allDataMain,
          ...allData,
  ...
  return (
    <div
      ...
      className="container-fluid px-0 container-pages text-right custom-create-auction"
    >
      <div className="panel-container newauction">
            ...
            {listComponent.map((item, i) => (
              <li
                ...
                className={`${selectComponent === item?.value && "current"} ${
                  selectComponent > item?.value && "done"
                }`}
              >
        ...
        <div className="text-left">
          {selectComponent !== 1 ? (
            <Button
              type="button"
              className="btn btn-warning btn-warn-custom mt-4"
              loading={loading}
              onClick={() => {
                dispatch(removeAUCTION());
              }}
