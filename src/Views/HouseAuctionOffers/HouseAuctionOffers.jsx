import React , {useState , useEffect} from 'react'
import {Input, Breadcrumb , Image , notification} from 'antd';
import {NavLink , Link} from 'react-router-dom';
import {BASE_URL} from '../../utils';
import axios from "../../utils/request";
import TableOffersHouseAuction from './TableOffersHouseAuction';


function HouseAuctionOffers(props) {

    const [houseAuctionOffersList, setHouseAuctionOffersList] = useState([]);
    const [artwork, setArtwork] = useState({});

    useEffect(() => {
        // axios.get(`${BASE_URL}/`).then(res => {
        //     console.log(res.data);
        //     setHouseAuctionOffersList(res.data.data.results)

        // }).catch(err => {
        //     console.log(err);
        // })


        axios.get(`${BASE_URL}/artworks/${props.match.params.id}/`).then(res => {
            console.log(res.data);
            setArtwork(res.data.data.result)

        }).catch(err => {
            console.log(err);
        })

    }, []);


    const openNotification = (state) => {
        notification.success({
          message: state ?  'تایید درخواست' : 'رد درخواست',
          description:  state ? `تایید درخواست با موفقیت انجام شد`  :  `رد درخواست با موفقیت انجام شد`,
            duration: 1.2,
            className: 'custom-class',
            style : {
                backgroundColor : '#f9faf5'
            }
        });
      };


    return (
        <React.Fragment>

                <div
                    style={{
                    marginTop: '30px'
                }}
                    className="container-fluid px-0 container-pages">

                    <div className="row m-0">
                        <div className="col">
                            <div className="row justify-content-start pb-3 mx-0">
                                <div className="col px-0">
                                    <div className="d-flex">
                                        <Breadcrumb>
                                            <Breadcrumb.Item>
                                                <NavLink key="1" onClick={e => props.toggleActiveNavDrawer("1")} to="/">
                                                    خانه
                                                </NavLink>
                                            </Breadcrumb.Item>
                                            <Breadcrumb.Item></Breadcrumb.Item>
                                            <Breadcrumb.Item>
                                                {` خانه حراجی پیشنهادی برای  ${props.match.params.name}`}
                                            </Breadcrumb.Item>
                                        </Breadcrumb>
                                    </div>
                                </div>
                            </div>

                            <div className="row content-page ">

                                <div className="col pt-5">

                                    <div className="d-block d-lg-flex ">
                                        <div className="col-12 col-lg-2">
                                            <p className="text-right">تصویر اثر</p>
                                        </div>
                                        <div className="col-12 col-lg-8">
                                            <div className="d-flex">

                                                <Image
                                                    width={200}
                                                    height={200}
                                                    src={artwork?.media ? artwork?.media?.exact_url : 'error'}
                                                    fallback="data:image/png;base64,"/>

                                            </div>
                                        </div>
                                    </div>

                                    <div className="d-block d-lg-flex mt-5">
                                        <div className="col col-lg-4">
                                            <p className="mb-2 mb-lg-0 text-right">نام کاربر درخواست کننده</p>
                                        </div>
                                        <div className="col">
                                            <Input value={artwork?.mobile} />
                                        </div>
                                        <div className="col-0 col-lg-2">

                                        </div>
                                    </div>

                                    <div className="d-block d-lg-flex mt-4">
                                        <div className="col col-lg-4">
                                            <p className="mb-2 mb-lg-0 text-right">نام اثر</p>
                                        </div>
                                        <div className="col">
                                            <Input value={"نام کاربر"} />
                                        </div>
                                        <div className="col-0 col-lg-2">

                                        </div>
                                    </div>

                                    <div className="d-block d-lg-flex mt-4">
                                        <div className="col col-lg-4">
                                            <p className="mb-2 mb-lg-0 text-right">نام هنرمند</p>
                                        </div>
                                        <div className="col">
                                            <Input value={artwork?.mobile} />
                                        </div>
                                        <div className="col-0 col-lg-2">

                                        </div>
                                    </div>

                                    <div className="d-block d-lg-flex mt-4">
                                        <div className="col col-lg-4">
                                            <p className="mb-2 mb-lg-0 text-right">تخمین قیمت</p>
                                        </div>
                                        <div className="col">
                                            <Input value={artwork?.mobile} />
                                        </div>
                                        <div className="col-0 col-lg-2">

                                        </div>
                                    </div>

                                    

                                    <div className="d-block d-lg-flex mt-4">
                                        <div className="col col-lg-4">
                                            <p className="mb-2 mb-lg-0 text-right">خانه‌های حراج جهت ارسال</p>
                                        </div>
                                        <div className="col">
                                            <Input value={artwork?.mobile} />
                                        </div>
                                        <div className="col-0 col-lg-2">

                                        </div>
                                    </div>



                                    <div className="d-flex mt-5">
                                        <TableOffersHouseAuction  
                                            houseAuctionOffersList={houseAuctionOffersList} 
                                            openNotification={openNotification}
                                        />
                                    </div>

                         
                          
                                </div>
                            </div>

                        </div>
                    </div>
                </div>


                
            </React.Fragment>
    )
}

export default HouseAuctionOffers;
